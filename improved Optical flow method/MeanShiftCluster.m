...
while numInitPts
    ...
    while 1     %loop untill convergence
        ...
        if norm(myMean-myOldMean) < stopThresh
            ...
            for cN = 1:numClust
                ...
                if distToOther < bandWidth/2                    %如果其带宽内/ 2合并新旧
                    mergeWith = cN;
                    break;
                end
            end
            ...
            if mergeWith > 0    % 合并的东西
                clustCent(:,mergeWith)       = 0.5*(myMean+clustCent(:,mergeWith));             %将最大值记录为两者合并的平均值（我知道偏向新值）
                %clustMembsCell{mergeWith}    = unique([clustMembsCell{mergeWith} myMembers]);   %record which points inside 
                clusterVotes(mergeWith,:)    = clusterVotes(mergeWith,:) + thisClusterVotes;    %将这些投票添加到合并的群集
            else    %its a new cluster
                numClust                    = numClust+1;                   %增加群集
                clustCent(:,numClust)       = myMean;                       %记录下平均值 
    ...
    initPtInds      = find(beenVisitedFlag == 0);           %我们可以对尚未访问的点进行初始化
    numInitPts      = length(initPtInds);                   %集合中的活动点数目

end

[val,data2cluster] = max(clusterVotes,[],1);                %一个点属于票数最多的集群
